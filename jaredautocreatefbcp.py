...
\x1b[1;94m     
\x1b[1;93m
\x1b[1;92m         LIBRE BASTA IKAW OHHH MISSYOU
\x1b[1;91m-----------------------------------------------
\x1b[1;97m> Author : MISSYOU
\x1b[1;97m> Github : https://github.com/SHOOTER-MAKER
\x1b[1;97m> Facebok: MISSYOU 
\x1b[1;97m> Version: OPEN SOURCE TO AUTO CREATE FB
\x1b[0;97m-----------------------------------------------"""
...
class create:
    ...
    def start(self):
        ...
        for x in range(lim):
            self.loop += 1
            sys.stdout.write(f'\r {OO}[MISSYOU CREATE-FB] {OO}{self.loop}/{str(lim)} OK:{len(ok)} - CP:{len(cp)}{OO} '),
            sys.stdout.flush()
            ...
            try:
                ...
                if 'checkpoint' in getts.url:
                    ...
                    print ('\r\033[1;33m[JARED-CP] '+cok['c_user']+' | '+passw+'\033[0;97m     ')
                        ...
                        for x in BeautifulSoup(po.text, 'html.parser').find_all('form', {'method': 'post'}):
                            if 'confirmation_event_location=cliff' in str(x):
                                ...
                                if 'checkpoint' in rex.url:
                                    cok = self.ses.cookies.get_dict()
                                    cp.append(email+passw)
                                    print ('\r\033[1;33m[CP TO MISSYOU] '+cok['c_user']+' | '+passw+'\033[0;97m     ')
                                else:
                                    coki = (";").join([ "%s=%s" % (key, value) for key, value in self.ses.cookies.get_dict().items() ])
                                    cok = self.ses.cookies.get_dict()
                                    print ('\r\033[1;32m[OK TO MISSYOU] '+cok['c_user']+' | '+passw+' | '+coki+'\033[0;97m     ')
